...
# st.write("📧 Using service account:", st.secrets["gdrive"]["client_email"])
...
# spreadsheet = gc.open_by_key("177G-qeI5NjAPEU4xqLJ_WPZ2xyHdAk-ZaU3QwMmDuQA")  # Replace with actual Sheet ID
...
def log_user_question_to_sheet(question):
    timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    sheet.append_row([timestamp, question])

# Check for user input (avoid using st.session_state.user_question unnecessarily)
if user_question:
    user_question = user_question.strip()

    # Log the question to Google Sheets
    log_user_question_to_sheet(user_question)
